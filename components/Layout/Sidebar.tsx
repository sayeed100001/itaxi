...
import { Home, Clock, Wallet, Settings, LogOut, Car, BarChart3, Users, LifeBuoy, Map, Bell, DollarSign, Moon, Sun, Sliders } from 'lucide-react';
        ...
        switch (currentRole) {
            case 'driver':
                return [
                    ...
                    { id: 'earnings', icon: <BarChart3 size={20} />, label: t.nav.earnings },
                    { id: 'support', icon: <LifeBuoy size={20} />, label: t.nav.support },
                    { id: 'profile', icon: <Car size={20} />, label: t.nav.vehicle },
                    ...commonLinks
                ];
            case 'admin':
                return [
                    { id: 'home', icon: <BarChart3 size={20} />, label: t.nav.dashboard },
                    { id: 'trips', icon: <Car size={20} />, label: t.nav.rides },
                    { id: 'drivers', icon: <Users size={20} />, label: t.nav.driver_mgmt },
                    { id: 'activity', icon: <Users size={20} />, label: t.nav.users },
                    { id: 'finance', icon: <DollarSign size={20} />, label: t.nav.finance },
                    { id: 'analytics', icon: <BarChart3 size={20} />, label: t.nav.analytics },
                    { id: 'admin_settings', icon: <Sliders size={20} />, label: t.nav.system_config },
                    ...commonLinks
                ];
            default: // rider
                return [
        ...
        <aside className="hidden lg:flex flex-col w-72 bg-white/90 dark:bg-dark-950/95 backdrop-blur-2xl border-e border-dark-200 dark:border-white/5 h-screen transition-all duration-300 z-50">
            ...
            <div className="mt-auto p-6 border-t border-dark-200 dark:border-white/5 bg-dark-50/50 dark:bg-dark-900/30">
                    ...
                    <div className="flex-1 min-w-0 text-start">
                        <div className="text-sm font-bold text-dark-900 dark:text-white truncate">{user?.name || t.common.guest_user}</div>
                        <div className="text-xs text-brand-600 dark:text-brand-400 truncate capitalize font-medium">
                            {tx('common.account_label', { role: user?.role ? t.roles[user.role] : t.common.guest })}
                        </div>
                    </div>
                </div>
                <button 
